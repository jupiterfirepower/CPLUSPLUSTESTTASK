#pragma once
#ifndef SAFE_RECPTR_H
#define SAFE_RECPTR_H
...
namespace sf {
    ...
    class recursive_free_shared_mutex {
		std::atomic<bool> want_x_lock;
        //struct cont_free_flag_t { alignas(std::hardware_destructive_interference_size) std::atomic<int> value; cont_free_flag_t() { value = 0; } }; // struct from C++17
    ...
    using default_recursive_free_shared_mutex = recursive_free_shared_mutex<>;
    // ---------------------------------------------------------------
}


#endif // #ifndef SAFE_RECPTR_H
